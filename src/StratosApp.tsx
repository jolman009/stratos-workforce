            ...
            <div className="mb-10">
              ...
              <div className="space-y-4">
                {notifications.filter((notification) => notification.unread).map((notification) => (
                  <NotificationItem key={notification.id} notification={notification} onAction={handleNotificationAction} onOpen={handleNotificationOpen} />
                ))}
              </div>
            </div>
            <div>
          ...
          <Layout currentScreen="profile" onNavigate={navigateTo} user={user} title="Profile" banner={mutationBanner}>
            <section className="relative bg-surface-container-lowest rounded-xl p-8 flex flex-col items-center text-center overflow-hidden border border-outline-variant/10">
              ...
              {isEditingProfile ? (
                <div className="w-full max-w-xs space-y-2">
                  <input className="w-full text-center text-2xl font-extrabold text-primary bg-surface-container-highest rounded-lg px-2 py-1 border-none focus:ring-2 focus:ring-primary/20" value={editForm.name} onChange={(event) => setEditForm({ ...editForm, name: event.target.value })} />
                </div>
              ) : (
            ...
            <section className="space-y-4 mt-8">
              <div className="flex items-center justify-between px-2">
                ...
                {!isEditingProfile ? (
                  <button type="button" onClick={() => { setEditForm({ name: user.name, phone: user.phone }); setIsEditingProfile(true); }} className="text-sm font-semibold text-primary hover:underline transition-all">
                    Edit
                  </button>
                ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-surface-container-low p-6 rounded-xl space-y-2 border border-outline-variant/10">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isEditingProfile ? (
                      <input className="flex-1 bg-surface-container-highest rounded-lg px-3 py-1 border-none focus:ring-2 focus:ring-primary/20 font-medium text-on-surface" value={editForm.phone} onChange={(event) => setEditForm({ ...editForm, phone: event.target.value })} />
                    ) : (
            ...
            <section className="space-y-4 mt-8">
              ...
              <div className="bg-surface-container-highest/30 backdrop-blur-md rounded-xl divide-y divide-outline-variant/10 border border-white/20">
                ...
                <button type="button" onClick={() => navigateTo('history')} className="w-full flex items-center justify-between p-5 hover:bg-surface-container-low transition-colors group text-left">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center shadow-sm">
            ...
            <section className="pt-8 mb-12">
              ...
              <button type="button" onClick={handleLogout} className="w-full py-5 bg-tertiary text-white rounded-xl font-bold text-lg flex items-center justify-center gap-3 shadow-lg shadow-tertiary/20 hover:bg-tertiary-container transition-all duration-300 active:scale-95">
                <LogOut size={20} />
                Logout
              </button>
            </section>
